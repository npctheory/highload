...
_main() {
    ...
    echo "host replication replicator $PG_NET md5" >> "$PG_HBA_CONF"

    sed -i "s/^#*ssl =.*/ssl = off/" "$PGDATA/postgresql.conf"
    sed -i "s/^#*wal_level =.*/wal_level = replica/" "$PGDATA/postgresql.conf"
    sed -i "s/^#*max_wal_senders =.*/max_wal_senders = 4/" "$PGDATA/postgresql.conf"
